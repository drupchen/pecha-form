...
def _rewrite_prefix(conn, src: str, new_path: str) -> None:
    ...
    conn.execute(
        ...
        "WHERE text_group = ? OR text_group LIKE ?",
        (new_path, tail_from, src, like),
    )
    affected = [
        r["path"] for r in conn.execute(
            "SELECT path FROM text_groups WHERE path = ? OR path LIKE ?", (src, like)
        ).fetchall()
    ]
    ...
    pos_by_path = {
        r["path"]: r["position"] for r in conn.execute(
            "SELECT path, position FROM text_groups WHERE path = ? OR path LIKE ?", (src, like)
        ).fetchall()
    }
    for old in affected:
        conn.execute("DELETE FROM text_groups WHERE path = ?", (old,))
    for old in affected:
        rewritten = new_path + old[len(src):]
        conn.execute(
            "INSERT OR IGNORE INTO text_groups(path, position) VALUES (?, ?)",
            (rewritten, pos_by_path.get(old)),
        )


def _direct_children(conn, parent: Optional[str]) -> List[str]:
    ...
    known = {r["path"] for r in conn.execute("SELECT path FROM text_groups").fetchall()}
    known |= {
        r["text_group"] for r in conn.execute(
            "SELECT DISTINCT text_group FROM texts WHERE text_group IS NOT NULL"
        ).fetchall()
    }
    children = set()
    for p in known:
    ...
    pos = {r["path"]: r["position"] for r in conn.execute(
        "SELECT path, position FROM text_groups").fetchall()}
...
def list_text_groups():
    conn = get_db()
    rows = conn.execute(f"SELECT path FROM text_groups {_ORDER_BY}").fetchall()
    conn.close()
...
def create_text_group(body: TextGroupCreate):
    ...
    conn = get_db()
    with conn:
        conn.execute("INSERT OR IGNORE INTO text_groups(path) VALUES (?)", (path,))
    rows = conn.execute("SELECT path FROM text_groups ORDER BY path").fetchall()
    conn.close()
    return [r["path"] for r in rows]
...
def move_text_group(body: TextGroupMove):
    ...
    conn = get_db()
    with conn:
        _rewrite_prefix(conn, src, new_path)
    rows = conn.execute(f"SELECT path FROM text_groups {_ORDER_BY}").fetchall()
    conn.close()
    return [r["path"] for r in rows]
...
def reorder_text_group(body: TextGroupReorder):
    ...
    with conn:
        if new_path != src:
            ...
            clash = conn.execute(
                "SELECT 1 FROM text_groups WHERE path = ? "
                "UNION SELECT 1 FROM texts WHERE text_group = ? OR text_group LIKE ? LIMIT 1",
        ...
        siblings = [s for s in _direct_children(conn, parent) if s != new_path]
        ...
        for i, path in enumerate(siblings):
            conn.execute("INSERT OR IGNORE INTO text_groups(path) VALUES (?)", (path,))
            conn.execute("UPDATE text_groups SET position = ? WHERE path = ?", (i, path))
...
def delete_text_group(body: TextGroupDelete):
    ...
    conn = get_db()
    in_use = conn.execute(
        "SELECT 1 FROM texts WHERE text_group = ? OR text_group LIKE ? LIMIT 1",
        (path, like),
    ).fetchone()
    if in_use:
        conn.close()
    ...
    with conn:
        conn.execute("DELETE FROM text_groups WHERE path = ? OR path LIKE ?", (path, like))
    rows = conn.execute("SELECT path FROM text_groups ORDER BY path").fetchall()
    conn.close()
    return [r["path"] for r in rows]
