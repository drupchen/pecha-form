...
class FormatDocument:
    def __init__(self, template=None):
        ...
        #     par styles
        self.s_title = 'EN - HEADING 1 (ENGLISH)'
        self.s_sections = 'EN - HEADING 2 (ENGLISH)'
        self.s_trans = 'EN - TRANSLATION (ENGLISH)'
        self.s_sml = 'EN - TRANSLATION - instructions (ENGLISH)'
        self.s_phon = 'FONÉTICA (FONÉTICAS)'
        self.s_mtr = 'MANTRA SKT'
        #    char styles
        self.s_char_trans = ''
        self.s_char_sml = ''
        self.s_char_phon = ''
        ...
        for s, parts in parsed_content:
            for part in parts:
                ...
                if part['skt']:
                    ...
                    par.add_run(part['skt'], style=self.s_char_mtr)
                    par = self.document.add_paragraph(style=self.s_mtr)
                    par.add_run('MANTRA')

                # translation
                par = self.document.add_paragraph(style=self.bklt_par_styles[s])
                for string in part['trans']:
                    ...
                    if "retrieve" in string:
                        tib = '\n'.join([p['tib'] for p in parts])
                        self.document.add_paragraph(tib)
